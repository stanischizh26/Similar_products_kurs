function editRow(editButton) {
    ...
    if (isEditing) {
        ...
        const updatedData = {
            ...
            plannedVolume: cells[2].innerText.trim(),
            actualVolume: cells[3].innerText.trim(),
...
function openSelectModal(cell) {
    const type = cell.dataset.field === "enterprise" ? 'enterprise' : 'product';
...
function selectItem(item, cell, type) {
    if (type === 'enterprise') {
        cell.dataset.enterpriseId = item.id;
        cell.innerText = item.name;
    } else if (type === "product") {
        cell.dataset.productId = item.id;
        cell.innerText = item.name;
    }
